...
def kernelRBF(dataset):
    ...
    print('Mean Square Error of SVM RBF :', metrics.mean_squared_error(Y,ypred))
    print(f"Accuracy Score of SVM RBF : {result * 100:.5f}%")
    end=time.time()
    print("Testing Time:", end - start, "Sec")
'''
    svm = SVC(kernel='rbf', C=0.01, gamma=0.001)
    start=time.time()
    ...
    end2=time.time()
    print('Mean Square Error of SVM RBF :', metrics.mean_squared_error(Y_test, prediction))
    print(f"Accuracy Score of SVM RBF : {svm.score(X_test,Y_test)*100:.5f}%")
